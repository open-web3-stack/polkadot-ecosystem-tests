...
>(chain: Chain<TCustom, TInitStorages>, testConfig: TestConfig) {
  ...
  const createPureProxyTx = client.api.tx.proxy.createPure('Any', 0, 0)
  ...
  const addPureProxyOwnerTx = client.api.tx.proxy.addProxy(bob.address, 'Any', 0)
  ...
  const removePureProxyOwnerTx = client.api.tx.proxy.removeProxy(alice.address, 'Any', 0)
  ...
  proxyTx = client.api.tx.proxy.proxy(pureProxyAddress, null, transferCall)
  const failedResult = await sendTransaction(proxyTx.signAsync(alice))
  ...
  return {
    ...
    children: [
      ...
      },
      {
        ...
        testFn: async () => await pureProxyOwnershipChangeTest(chain, testConfig),
      },
    ],
  }
}

 ...
 * (`referencePreimages.json`) so it can be reviewed, hashed, and exercised end to end.
 *
 ...
 *      queues (V1 + V2), the outbound queue, the beacon client, and both Ethereum gateways (V1 + V2).
 *      This stops messages in both directions and freezes Ethereum consensus tracking.
 *   2. Sets the Asset Hub Snowbridge frontend export operating mode to `Halted`, blocking new exports.
 *   3. Overwrites both export base-fee storage items (V1 + V2) with `u128::MAX`, so any export that
 *      slips through is economically impossible. This is defense-in-depth on top of step 2.
 *
...
/** Largest u128 value; the halt preimage sets the bridge base fees to this to make exports prohibitively expensive. */
const MAX_U128 = (1n << 128n) - 1n

// Asset Hub storage keys for the Snowbridge export base fee, addressed directly because they are not
// exposed as typed storage items. Each is `twox_128(":BridgeHubEthereumBaseFee:")` (V1) and

...
>(chain: Chain<TCustom, TInitStorages>, testConfig: TestConfig) {
  ...
  const encodedCallList = Array(3).fill(encodedCall)
  ...
  // If the request generated a "Transfer" event, then so will the unrequest.
  if (hasBalanceEvents) {
    expect(events.length).toBe(1)
    assert(client.api.events.balances.Transfer.is(events[0].event))
  } else {
...
>(chain: Chain<TCustom, TInitStorages>) {
  ...
  console.info(
    `Max preimage size: ${maxPreimageSize} bytes and oversized preimage size: ${oversizedBytes.length} bytes`,
  )
  ...
  // TODO: Verify that no preimage events were emitted, and that an ExtrinsicFailed event was emitted. Currently, the ExtrinsicFailed event is not being captured as expected.
  // expect((await getEventsWithType(client, 'preimage')).length).toBe(0)
  // expect((await getEventsWithType(client, 'system')).length).toBeGreaterThan(0)
...
>(chain: Chain<TCustom, TInitStorages>, oldPreimagesCount: number, newPreimagesCount: number) {
  ...
  // TODO: How to check for fees properly?
  console.info(
    `Expect fees: ${expectFees}, free before: ${aliceFreeFundsAfterNote}, free after: ${aliceFreeFundsAfterUpdate}`,
  )

  /*
  // 6. If the ratio of new to total preimages is less than 90%, check that fees have been paid.
  if (expectFees) {
    expect(aliceFreeFundsAfterUpdate).toBeGreaterThan(aliceFreeFundsAfterNote)
  } else {
    expect(aliceFreeFundsAfterUpdate).toBe(aliceFreeFundsAfterNote)
  }
  */
}

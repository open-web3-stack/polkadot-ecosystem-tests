...
>(chain: Chain<TCustom, TInitStorages>, addressEncoding: number, proxyType: number) {
  ...
  let otherSignatories = [pureProxyAddress, charlie.address].sort()
...
>(chain: Chain<TCustom, TInitStorages>, addressEncoding: number, proxyType: number) {
  ...
  // Check that the proxy's deposit is gone.
...
>(chain: Chain<TCustom, TInitStorages>, addressEncoding: number, proxyType: number) {
  ...
  await setupBalances(client, [
    { address: alice.address, amount: 100e10 },
    { address: bob.address, amount: 30e10 },
    { address: charlie.address, amount: 0 },
  ...
  // Alice creates the multisig for announcing the transaction.
  ...
  // Bob co-signs the multisig, which allows it to announce the transfer.
  ...
  // Dave now rejects the announced transfer.
  ...
  // The multisig re-announces the transfer call, with Alice signing first.
  currBlockNumber = (await client.api.rpc.chain.getHeader()).number.toNumber()
...
>(chain: Chain<TCustom, TInitStorages>, addressEncoding: number, proxyType: number, proxyDelay: number) {
  ...
  // Dave assigns the multisig as his proxy with a delay.
  ...
  do {
    ...
    if (proxyDelay < 0) {
      ...
      // Check that Alice has not received the funds yet.
...
>(
  ...
  return {
    ...
    children: [
      ...
      {
        kind: 'test',
        label: '2-of-3 multisig as standard proxy announcement and rejection',
        testFn: () => multisigAsStandardProxyAnnouncementTest(chain, testConfig.addressEncoding, proxyTypes['Any']),
      },
      {
        kind: 'test',
        label: '2-of-3 multisig as standard proxy announcement with delay',
        testFn: () =>

...
>(chain: Chain<TCustom, TInitStorages>, addressEncoding: number, proxyType: number) {
  ...
  let otherSignatories = [pureProxyAddress, charlie.address].map((addr) => encodeAddress(addr, addressEncoding)).sort()
  ...
  otherSignatories = [alice.address, charlie.address].map((addr) => encodeAddress(addr, addressEncoding)).sort()
...
async function multisigAsStandardProxyTest<
  TCustom extends Record<string, unknown> | undefined,
  TInitStorages extends Record<string, Record<string, any>> | undefined,
>(chain: Chain<TCustom, TInitStorages>, addressEncoding: number, proxyType: number, expectTransfer: boolean) {
  const [client] = await setupNetworks(chain)
  ...
  const threshold = 2
  const maxWeight = { refTime: 1000000000, proofSize: 1000000 } // Conservative weight limit
  let otherSignatories = [bob.address, charlie.address].map((addr) => encodeAddress(addr, addressEncoding)).sort()
...
>(chain: Chain<TCustom, TInitStorages>, addressEncoding: number, proxyType: number) {
  ...
  let otherSignatories = [bob.address, pureProxyAddress].map((addr) => encodeAddress(addr, addressEncoding)).sort()
  ...
  otherSignatories = [alice.address, bob.address].map((addr) => encodeAddress(addr, addressEncoding)).sort()
  ...
  otherSignatories = [dave.address, eve.address].map((addr) => encodeAddress(addr, addressEncoding)).sort()
  ...
  otherSignatories = [charlie.address, dave.address].map((addr) => encodeAddress(addr, addressEncoding)).sort()
...
>(chain: Chain<TCustom, TInitStorages>, addressEncoding: number, proxyType: number) {
  ...
  // Check that the pure proxy was created successfully.
  const pureProxyAddress = await getAndVerifyPureProxyAddress(client, alice, addressEncoding)
  ...
  const threshold = 2
  const maxWeight = { refTime: 1000000000, proofSize: 1000000 } // Conservative weight limit
  const otherSignatories = [bob.address, charlie.address].map((addr) => encodeAddress(addr, addressEncoding)).sort()
...
>(chain: Chain<TCustom, TInitStorages>, addressEncoding: number, proxyType: number) {
  ...
  const announcementObject = {
    real: encodeAddress(dave.address, addressEncoding),
    callHash: transferCall.method.hash.toHex(),
    height: currBlockNumber + 3,
  }
  ...
  otherSignatories = [bob.address, charlie.address].map((addr) => encodeAddress(addr, addressEncoding)).sort()
...
async function multisigAsStandardProxyAnnouncementWithDelayTest<
  TCustom extends Record<string, unknown> | undefined,
  TInitStorages extends Record<string, Record<string, any>> | undefined,
>(chain: Chain<TCustom, TInitStorages>, addressEncoding: number, proxyType: number, proxyDelay: number) {
  const [client] = await setupNetworks(chain)
  ...
  do {
    ...
    const [, multisigExtrinsicIndex2] = await getAndVerifyMultisigEventData(client, alice.address, addressEncoding)
    ...
    children: [
      {
        ...
        testFn: () => multisigWithPureProxyTest(chain, testConfig.addressEncoding, proxyTypes['Any']),
      },
      {
        kind: 'test',
        label: '2-of-3 multisig with pure proxy (non-transfer)',
        testFn: () => multisigWithPureProxyTest(chain, testConfig.addressEncoding, proxyTypes['NonTransfer']),
      },
      {
        kind: 'test',
        label: '2-of-3 multisig as standard proxy (any)',
        testFn: () => multisigAsStandardProxyTest(chain, testConfig.addressEncoding, proxyTypes['Any'], true),
      },
      {
        kind: 'test',
        label: '2-of-3 multisig with pure proxy multisig',
        testFn: () => multisigWithPureProxyMultisigTest(chain, testConfig.addressEncoding, proxyTypes['Any']),
      },
      {
        kind: 'test',
        label: 'Cancel 2-of-3 multisig with pure proxy before any other approvals',
        testFn: () => cancelMultisigWithPureProxyTest(chain, testConfig.addressEncoding, proxyTypes['Any']),
      },
      {
        kind: 'test',
        label: '2-of-3 multisig as standard proxy with announcement and rejection',
        testFn: () => multisigAsStandardProxyAnnouncementTest(chain, testConfig.addressEncoding, proxyTypes['Any']),
      },
      {
        kind: 'test',
        label: '2-of-3 multisig as standard proxy with announcement and delay',
        testFn: () =>
          multisigAsStandardProxyAnnouncementWithDelayTest(chain, testConfig.addressEncoding, proxyTypes['Any'], 3),
      },
    ],
  }
}
    ...
    children: [
      {
        ...
        testFn: () => multisigAsStandardProxyTest(chain, testConfig.addressEncoding, proxyTypes['NonTransfer'], false),
      },
    ],
  }
}
...
>(
  ...
  return {
    ...
    children: [
      successMultisigProxyE2ETests(chain, testConfig, proxyTypes),
      failureMultisigProxyE2ETests(chain, testConfig, proxyTypes),
    ],
  }
}

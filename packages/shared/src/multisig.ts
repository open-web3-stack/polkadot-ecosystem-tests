...
>(chain: Chain<TCustom, TInitStorages>, addressEncoding: number) {
  ...
  const finalApprovalTx = client.api.tx.multisig.asMulti(
    threshold,
    [alice.address, charlie.address].map((addr) => encodeAddress(addr, addressEncoding)).sort(),
    {
...
>(chain: Chain<TCustom, TInitStorages>, addressEncoding: number) {
  ...
  const approveTx2 = client.api.tx.multisig.asMulti(
    threshold,
    [bob.address, alice.address].map((addr) => encodeAddress(addr, addressEncoding)).sort(),
    {
      height: blockNumber + 1,
      index: multisigExtrinsicIndex,
    },
...
>(chain: Chain<TCustom, TInitStorages>, addressEncoding: number) {
  ...
  const threshold = 2
  const otherSignatories = [bob.address, charlie.address].map((addr) => encodeAddress(addr, addressEncoding)).sort() // Correct order
  const maxWeight = { refTime: 1000000000, proofSize: 1000000 }
  ...
  const finalApprovalTx = client.api.tx.multisig.asMulti(
    ...
    [charlie.address, alice.address]
      .map((addr) => encodeAddress(addr, addressEncoding))
      .sort()
...
>(chain: Chain<TCustom, TInitStorages>, addressEncoding: number) {
  ...
  const threshold = 2
  const otherSignatories = [alice.address, bob.address].map((addr) => encodeAddress(addr, addressEncoding)).sort() // Alice includes herself!
  const maxWeight = { refTime: 1000000000, proofSize: 1000000 }
...
>(chain: Chain<TCustom, TInitStorages>, addressEncoding: number) {
  ...
  const approveTx = client.api.tx.multisig.approveAsMulti(
    threshold,
    [alice.address, bob.address].map((addr) => encodeAddress(addr, addressEncoding)).sort(),
    {
      height: blockNumber + 1,
      index: multisigExtrinsicIndex,
    },

...
for (const chain of Object.keys(original)) {
	...
	const sameSet = probeSet.size === originalSet.size && [...probeSet].every((e) => originalSet.has(e))
	if (!sameSet) {
		process.stderr.write(`! ${chain}: probe and JSON endpoint sets differ; keeping original order\n`)
		reordered[chain] = originalEndpoints
		continue
	}
